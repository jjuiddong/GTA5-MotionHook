...
network::cUDPClient g_udpClient;
...
void update()
{
	...
	//-------------------------------------------------------------------------------------------------------------------
	// motion data
	float heading = ENTITY::GET_ENTITY_HEADING(PED::GET_VEHICLE_PED_IS_USING(playerPed));
	float roll = ENTITY::GET_ENTITY_ROLL(PED::GET_VEHICLE_PED_IS_USING(playerPed));
	float pitch = ENTITY::GET_ENTITY_PITCH(PED::GET_VEHICLE_PED_IS_USING(playerPed));
	Vector3 velocity = ENTITY::GET_ENTITY_SPEED_VECTOR(PED::GET_VEHICLE_PED_IS_USING(playerPed), true);
	...
	if (deltaT > 30)
	{
		...
		g_udpClient.SendData((BYTE*)&data, sizeof(data));		
	}
}

void main()
{	
	g_udpClient.Init("127.0.0.1", 10001, 10);
